...
- [CSS](#css)
  ...
  - [伪类](#伪类)
  - [伪元素](#伪元素)
  - [CSS 有哪些样式可以给子元素继承](#css-有哪些样式可以给子元素继承)
  - [css 解析顺序](#css-解析顺序)
  - [布局](#布局)
    - [盒间布局](#盒间布局)
    - [盒内布局](#盒内布局)
  ...
  - [BFC](#bfc)
  - [margin 相关](#margin-相关)
    - [外边距合并（margin_collapsing）](#外边距合并margin_collapsing)
      - [发生情况](#发生情况)
      - [如何避免](#如何避免)
  - [rem 和 em](#rem-和-em)
  - [viewport](#viewport)
  - [例题](#例题)
    - [visibility: hidden 和 display: none 的区别](#visibility-hidden-和-display-none-的区别)
    - [垂直水平居中](#垂直水平居中)
    - [zoom 和 scale 的区别](#zoom-和-scale-的区别)
    - [CSS 中 transition 和 animate 有何区别? animate 如何停留在最后一帧](#css-中-transition-和-animate-有何区别-animate-如何停留在最后一帧)
    - [Canvas 优化](#canvas-优化)
    - [CSS3 的新特性](#css3-的新特性)
    - [CSS 硬件加速](#css-硬件加速)
...
元素按照其在 HTML 中的位置顺序决定排布的过程。并且这种过程遵循标准的描述。默认为`static`（忽略 top, bottom, left, right 或者 z-index 声明）。

`relative`：设置了相对定位之后，通过修改 top，left，bottom，right 值，元素会在自身文档流所在位置上被移动，其他的元素则**不会调整位置**来弥补它偏离后剩下的空隙。

`absoulute`：设置的元素会脱离正常流。偏移是相对于是它最近的设置了定位属性（position 值不为 static）的元素。其他文档元素会**弥补留下位置**。

`fixed`：设置的元素会脱离正常流。偏移位置相对于浏览器的可视窗口。
...
`::after`: 用来创建一个伪元素，做为已选中元素的最后一个子元素。

`::before`: 用来创建一个伪元素，做为已选中元素的第一个子元素。

`::selection`: 使用鼠标或其他选择设备选中的部分。

`::first-line`: 某块级元素的第一行应用样式。
...
2. 行内元素: `display:inline`和`display:inline-block`
...
.box .left {
  float: left;
  width: 100px;
  margin-left: -100%; /*100%是根据父元素宽度进行计算*/
  background-color: red;
  position: relative;
  left: -100px; /*往左拉*/
  height: 100%;
...
5. 普通流中的块级元素的 margin-bottom 永远和它相邻的下一个块级元素的 margin-top 叠加，除非相邻的兄弟元素有空隙。
...
`em`根据当前字体大小来计算，如果没有设置会根据父元素的字体大小来计算。
...
3. zoom 会真实改变元素大小，会引起回流。而 scale 只发生重绘。
4. 对文字的缩放规则不一致。zoom 缩放依然受限于最小 12 像素中文大小限制；而 scale 就是纯粹的对图形进行比例控制，文字为 50%原来尺寸。
